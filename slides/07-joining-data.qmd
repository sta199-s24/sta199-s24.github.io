...
-   Do the exam review over (to be posted by the weekend)
...
Today we would like to answer a specific question - what was the passenger capacity (i.e. maximum number of passengers) that could have flown out of the three airports in 2013?
...
left_join(
  ...
  group_by(origin) |>
  summarize(
    capacity = sum(seats, na.rm = FALSE)
  )
```
