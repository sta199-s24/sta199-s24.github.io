...
Calculate the number of counties in each state and display your results in descending order of number of counties, and how many?
Which state has the highest number of counties?
Then, display your results in ascending order.
Which state has the lowest number of counties, and how many?
...
Discover the most popular county names by counting how many times each name occurs in the dataset and displaying the results in order of descending counts.
...
In this question part, we want you to revisit this box plot and identify
...
Your response should be a data frame with one row and five columns: county name, state name, population density, total population, and area, in this order.
If your response has multiple rows, it the data frame should be arranged in descending order of population density.
...
> A typical Midwestern county has population density of \_\_\_\_ per unit area.
> The middle 50% of the counties have population densities between \_\_\_ to \_\_\_.
...
ggplot(
  ...
  annotate(
    geom = "point",
    x = 7.35, y = 48.5, color = "red", shape = "square open", 
    size = 6, stroke = 2, fill = "pink"
  ) +
  annotate(
    geom = "point",
    x = 45, y = 10, color = "red", shape = "square open", 
    size = 30, stroke = 2, fill = "pink"
  ) +
  theme_minimal()
```

a\.
In a single pipeline, identify the observations marked in red squares in the plot above.
...
-   `Yes` if the point is one the ones you identified in part a, i.e., one of the points marked in red squares in the plot above.
...
In a single pipeline, calculate the total population for each state, and display the results in descending order of total population.
Which Midwestern state is most populous?
...
First, display the `state_poverty` data frame in descending order of `mean_percbelowpoverty`.
Which state has the highest average percentage below poverty across its counties?

Then, display it in ascending order of `mean_percbelowpoverty`.
...
The lab is graded out of a total of 40 points.

On Questions 1 through 9, you can earn up to 4 points on each question:

-   4: Response shows proficiency and addresses all or almost all of the rubric items.
-   3: Response shows understanding and addresses majority of the rubric items.
...
On Question 10, you can earn either 0 or 4 points for selecting questions and pages on Gradescope:

-   4: Selected questions and pages on Gradescope.
