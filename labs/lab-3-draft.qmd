...
In this lab you'll build the data wrangling and visualization skills you've developed so far and data tidying to your repertoire.
...
What do the numeric values in the data frame represent?
...
Calculate the difference between 2021 inflation rates and 1993 inflation rates. Which country's inflation change is the largest over this time period?
...
Create a vector called `countries_of_interest` which contains the names of countries you want to visualize the inflation rates for over the years.
...
In a single pipeline, filter your reshaped dataset to include only the `countries_of_interest` from the previous exercise and create a plot of annual inflation vs. year for these countries.
...
Add a column to the `us_inflation` dataset called `description` which has the CPI division description that matches the `cpi_division_id`, by joining the two datasets.
