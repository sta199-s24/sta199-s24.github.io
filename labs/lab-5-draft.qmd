...
a.  For each of the character strings below, determine if the string is a proper code chunk label.
    If not, explain why and fix it so the document renders.
...
b.  In a single pipeline starting with `survey_counts`, calculate the proportions of individuals in this sample who are planning to vote for each of the candidates (or are undecided) **among those who are 18-49 years old** as well as **among those who are 50+ years old**.
    Your response should be an **ungrouped** data frame with a row for each `age` / `vote` combination, just like `survey_counts`, but should also include the proportions.
    Save this data frame as `survey_proportions` and display its contents.
    Then, in 1 sentence, comment on how these proportions compare to each other.

## Exercise 9

a.  Starting with your `survey_proportions` data frame from the previous exercise, re-create the following visualization that displays relationship between `age` and `vote`.
