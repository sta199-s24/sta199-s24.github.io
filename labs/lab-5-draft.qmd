...
a.  What values does each of the following chunk options take and what do they do?
    ...
    To answer this question I recommend adding each of the chunk options, one at a time, and re-rendering the document with some sample code in the chunk.
...
b.  What do the chunk options `fig-height` and `fig-width` do -- what do they do when they're set in a single code chunk and what do they do when they're set in the document YAML on top?
    ...
    To answer this question I recommend creating two code chunks with the same plotting code in them, re-rendering the document with different values for these chunk options in the chunk YAML and then different values in the document YAML.
